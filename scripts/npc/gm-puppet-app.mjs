...
export class GmPuppetApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    /** Toggle browser speech recognition (mic dictation). */
    handleMic() {
        ...
        // Don't start the mic while audio is playing — would just pick up
        // its own NPC speech and feed it back into the input.
        ...
        if (!RecognitionClass) {
            ui.notifications.warn("Speech recognition not supported in this browser.");
            return;
        }
        ...
        let finalTranscript = "";
        r.onresult = (event) => {
            // Drop late events. If _recognition is null we've hard-stopped
            // (probably mid-send); if the input is disabled we're playing
            // back TTS audio. Either way we must NOT touch the input.
            if (!this._recognition || this._inputField?.disabled) return;
            ...
            if (this._inputField) {
                this._inputField.value = (finalTranscript + interim).replace(/\s+/g, " ").trimStart();
            }
        };
        r.onerror = (e) => {
            console.warn("ACE: Engine | GM puppet mic error:", e);
            this._recognition = null;
            this._micBtn?.classList.remove("active");
        };
        r.onend = () => {
            // Only act if this is still the active recognition — hardStop
            // detaches our handlers before stopping, so this should normally
            // never fire after a deliberate stop. Defensive cleanup anyway.
            if (this._recognition === r) {
                this._recognition = null;
                this._micBtn?.classList.remove("active");
                this._inputField?.focus();
            }
    ...
    /**
     * Cleanly tear down speech recognition. Detach the event handlers
     * BEFORE stopping so any in-flight onresult / onend events are
     * silently dropped — preventing late transcripts from being written
     * into the input after the user pressed send.
     */
    _hardStopMic() {
